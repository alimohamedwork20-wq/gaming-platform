...
export default function GamesCategory() {
  ...
  useEffect(() => {
    const getCategoryGames = async () => {
      try {
        ...
        console.error("خطأ أثناء جلب ألعاب القسم أو الـ Wishlist:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  // ➕ دالة تحديث الـ State عند نجاح الإضافة من جوه الكارت
          ...
          {loading ? (
            ...
            games.map((item) => (
              <GameCardKeyed
                key={item.id}
                game={item}
                isAdded={favGameIds.includes(item.id)} // 👈 شيك لو اللعبة متضافة
                onAddSuccess={onAddGameSuccess} // 👈 دالة التحديث الفوري
              />
            ))
          ) : (
            <div className="text-slate-400 p-6">No games found from API.</div>
          )}
        </div>
...
function GameCardKeyed({ game, isAdded, onAddSuccess }) {
  ...
  const navigate = useNavigate(); // 🌟 للتوجيه لصفحة الـ wishlist عند الضغط على View
  ...
  // 🔥 دالة هاندلر الأكشن الموحدة
  const handleWishlistAction = async (e) => {
    e.preventDefault();
    e.stopPropagation();

    // 1. لو متضافة بالفعل -> حوله لصفحة الـ Wishlist علطول
    if (isAdded) {
      navigate("/wishlist");
      return;
    }

    // 2. لو مش متضافة -> اضرب الـ API وسجلها في الباك إند والداتابيز
    const token = Cookies.get("token");
    if (!token) {
    ...
    try {
      ...
      if (res.status === 200 || res.status === 201) {
        ...
        onAddSuccess(game.id); // اقلب الزرار لـ View فوراً بدون ريفريش
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ width: "100%" }} className="game-card-img-wrapper">
          <img
            style={{ width: "100%" }}
            src={gameScreenshots[currentImgIndex]}
            alt={game.name}
          />
        </div>
        ...
        <div className="game-card-content">
          ...
          <div className="game-card-footer">
            <div className="platform-icons-container">
              {game.parentPlatforms ? (
                game.parentPlatforms
                  .split(", ")
                  .filter(
                    (platformName) => platformName.trim() !== "Apple Macintosh",
                  )
            ...
            {/* 🌟 الزرار الذكي والديناميكي بناءً على حالة الـ Wishlist */}
            <button
